...
| DB_URI                       | None or the URL of the PostgreSQL (`postgresql://postgres:ChangeIt@localhost:5432/geocontext`).                                                                                                                                                                             | None (use InMemorySaver)      |
| TOOL_SELECTOR_MODEL          | Lightweight model that dynamically selects the most relevant tools for each turn (avoids sending every tool schema to the main model). Set to an empty string or `none` to disable.                                                                                         | "anthropic:claude-haiku-4-5"  |
| TOOL_SELECTOR_MAX_TOOLS      | Maximum number of tools selected per turn (excluding `TOOL_SELECTOR_ALWAYS_INCLUDE`).                                                                                                                                                                                       | 5                             |
| TOOL_SELECTOR_ALWAYS_INCLUDE | Comma-separated tool names always exposed to the main model, regardless of the selection.                                                                                                                                                                                   | "create_map,geocode"          |
